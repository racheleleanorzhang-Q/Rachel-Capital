...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `20260728-01` | `2026-07-28` | `投资` | `买入` | `浪潮信息（000977.SZ）` | `第二增长引擎初始观察仓：买入浪潮信息200股` | `当首次建仓后的趋势延续、放量突破或放量跌破出现明确信号，或人工智能算力板块联动显著变化时复盘。` | `已执行` |
...
| 决策类型 | `买入` |
| 影响资产 | `浪潮信息（000977.SZ）` |
| 背景 | Rachel Capital 当前核心目标是在未来 `3-5个月` 内，通过核心持仓做T、仓位管理以及优质股票轮动，修复历史亏损并提高账户回本效率。浪潮信息此前已被纳入第二增长引擎候选观察范围，本次买入的目的，是正式建立人工智能算力方向的初始仓位，验证其在第二增长引擎中的趋势弹性和后续做T适配性。 |
| 决策内容 | 以 `82.20元` 买入浪潮信息 `200股`，成交金额 `16,440元`，未计交易费用，作为第二增长引擎初始观察仓。 |
| 决策原因 | 浪潮信息属于人工智能服务器与算力基础设施方向，是当前第二增长引擎候选资产之一。相比只停留在观察名单，本次买入使第二增长引擎进入实际持仓验证阶段。首次仅买入 `200股`，属于受控试仓，不代表一次性完成目标仓位。后续是否加仓，应结合趋势、成交量、板块强弱和支撑阻力重新决策；当前不预设必须补仓，也不得因为短期下跌而机械摊低成本。 |
| 仓位影响 | 新增浪潮信息 `200股`，初始投入金额 `16,440元`，未计交易费用。该仓位被归类为第二增长引擎初始观察仓，而不是做T交易仓，也不是核心修复仓。现金余额应以下一次账户总览统一校准后的可靠数据为准，本记录不单独估算最新现金余额。 |
| 成功标准 | 浪潮信息能够在首次建仓后表现出可跟踪的趋势延续性、板块联动性和后续做T适配性；同时该仓位保持为受控试仓，不因短期波动演变为无上限加仓。 |
| 复盘触发条件 | 当首次建仓后的趋势延续、放量突破或放量跌破出现明确信号，人工智能算力板块及同类标的联动发生显著变化，或后续需要对该仓位进行加仓、减仓或开展做T时复盘。 |
| 最终结果 | `已执行` |
| 经验总结 | 第二增长引擎的新仓位应先通过小规模真实持仓验证，而不是在缺乏持续性证据时一次性重仓。首次建仓本身不等于做T交易，后续任何加仓、减仓或做T都应形成新的明确决策记录。 |
